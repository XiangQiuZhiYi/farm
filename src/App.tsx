...
function TaskBoardModal({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="modalBackdrop" role="presentation" onClick={onClose}>
            <div
                className="modalBox taskBoardModal"
                role="dialog"
                aria-label="任务板"
            ...
            >
                ...
                >
                    <button
                        ...
                        className={
                            activeTab === "active"
                                ? "taskBoardTab taskBoardTabActive"
                                : "taskBoardTab"
                        }
                    ...
                    {activeTab === "offers" ? (
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="taskOfferHead">
                                                    ...
                                                    <span
                                                        className={`taskDifficultyBadge taskDifficulty-${task.difficulty}`}
                                                    >
                                                        {task.difficulty}
                                                    </span>
                                                </div>
                                                <ul className="taskRequirementList">
                                                    {task.requirements.map(
                                                        (requirement) => (
                                                            <li
                                                                key={`${offer.taskId}-${requirement.plantId}`}
                                                            >
                                                                <span>
                                                                    {getPlantById(
                                                                        requirement.plantId,
                        ...
                        <section className="taskSection">
                            ...
                            {activeTaskDefinitions.length > 0 ? (
                                <div className="taskOfferList">
                                    {activeTaskDefinitions.map(
                                        ({
                                            state: activeTaskState,
                                            definition: activeTaskDefinition,
                                        }) => (
                                            <article
                                                key={activeTaskState.taskId}
                                                className="taskOfferCard taskOfferCardActive"
                                            >
                                                <div className="taskOfferHead">
                                                    <div>
                                                        <h4>
                                                            {
                                                                activeTaskDefinition.title
                                                            }
                                                        </h4>
                                                        <p>
                                                            已接于{" "}
                                                    ...
                                                    <span className="taskStatusBadge">
                                                        已登记
                                                    </span>
                                                </div>
                                                <ul className="taskRequirementList">
                                                    {activeTaskDefinition.requirements.map(
                                                        (requirement) => {
                                                            const owned =
                                                                inventory[
                                                                    requirement
                                                            ...
                                                            return (
                                                                <li
                                                                    key={`${activeTaskState.taskId}-${requirement.plantId}`}
                                                                >
                                                                    <span>
                                                                        {getPlantById(
                                                                            requirement.plantId,
                                                ...
                                                <div className="taskOfferFooter">
                                                    <span>
                                                        完成奖励{" "}
                                                        {
                                                            activeTaskDefinition.rewardGold
                                                        }{" "}
                                                        金
                                                    </span>
                                                    <button
                                                        ...
                                                        className="toolbarBtn toolbarBtnPrimary"
                                                        disabled={
                                                            !activeTaskDefinition.requirements.every(
                                                                (requirement) =>
                                                                    (inventory[
                                                                        requirement
                                                                            .plantId
            ...
            >
                ...
                <ul className="expandList">
                    {REGION_CONFIGS.map((region) => {
                        ...
                        const current = plots.filter(
                            (p) => p.regionId === region.id,
                        ).length;
                        const full = current >= region.maxPlotCount;
                        // 获取该区域下所有土地类型
                        const landTypes = LAND_TYPE_CONFIGS.filter(
                            (l) => l.regionId === region.id,
                        );

                        return (
                            <li
                                key={region.id}
                                className={`expandItem ${locked ? "expandLocked" : ""}`}
                            >
                                <div className="expandRegionHeader">
                                    <span className="expandName">
                                        {region.name}
                                    </span>
                ...
                <p className="expandGold">
                    当前金币：{economy.gold.toFixed(0)} 金
                </p>
            </div>
        </div>
    );
                ...
                <div className="saveModalBody">
                    ...
                    <div>
                        ...
                        <div className="saveCreateGroup">
                            <button
                                type="button"
                                className="saveCreateCard"
                                onClick={() => onCreate("real")}
                            >
                                <span className="saveCreateCardTitle">
                                    <span style={{ color: "#8ce0a1" }}>●</span>{" "}
                                    真实档
                                </span>
                                <span className="saveCreateCardDesc">
                                    时间 1:1 流逝，无法加速。适合正式体验。
                                </span>
                            </button>
                            <button
                                type="button"
                            ...
                            <button
                                type="button"
                                className="saveCreateCard saveCreateCardSandbox"
                                onClick={() => onCreate("sandbox")}
                            >
                                <span className="saveCreateCardTitle">
                                    <span style={{ color: "#c878ff" }}>●</span>{" "}
                                    全解锁档
                                </span>
                                <span className="saveCreateCardDesc">
                                    10万金币、全部植物和地块已解锁、支持加速。适合自由探索。
                                </span>
                            </button>
                        </div>
                    </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="saveItemBadge">
                                                <span
                                                    className={`saveTypeDot ${slot.type}`}
                                                />
                                                <span className="saveTypeLabel">
                                                    {slot.type === "real"
                                                        ? "真实"
                                                        : slot.type ===
                                                            "sandbox"
                                            ...
                                            <div className="saveItemInfo">
                                                <div className="saveItemNameRow">
                                                    {rs?.editing ? (
                                                        ...
                                                        <>
                                                            <p className="saveName">
                                                                {slot.name}
                                                            </p>
                                                            {isActive && (
                                                                <span className="saveActiveBadge">
                                                                    当前
                                                                </span>
                                                            )}
                                                        </>
...
function App() {
    ...
    useEffect(() => {
        ...
        async function initializeSaves() {
            if (!electronRuntime) {
                setSaveError(
                    "当前为 Web 运行环境，存档管理仅在 Electron 本地版可用。",
                );
            ...
            try {
                ...
                if (cancelled) return;
                setSaveError(
                    error instanceof Error ? error.message : "存档初始化失败",
                );
            } finally {
                if (!cancelled) {
                    setSaveLoading(false);
    ...
    useEffect(() => {
        ...
        const unsubscribe = useGameStore.subscribe((state, prevState) => {
            const changed =
                ...
                state.economy.cumulativeEarned !==
                    prevState.economy.cumulativeEarned;
            if (!changed) return;
    ...
    async function handleCreateSlot(type: SaveSlotType) {
        try {
            ...
            setSaveError(
                error instanceof Error ? error.message : "创建存档失败",
            );
        }
    }
    ...
    async function handleSwitchSlot(slotId: string) {
        try {
            ...
            setSaveError(
                error instanceof Error ? error.message : "切换存档失败",
            );
        }
    }
    ...
    async function handleDeleteSlot(slotId: string) {
        ...
        try {
            ...
            setSaveError(
                error instanceof Error ? error.message : "删除存档失败",
            );
        }
    }
    ...
    return (
        <div className={sidebarCollapsed ? "shell shellCollapsed" : "shell"}>
            <aside
                className={
                    sidebarCollapsed ? "sidebar sidebarCollapsed" : "sidebar"
                }
            >
                <div className="brand">
                    <div className="brandRow">
                        {!sidebarCollapsed ? (
                            <div className="brandText">
                                ...
                                <span className="brandSub">FARM</span>
                            </div>
                        ) : null}
                        <button
                            type="button"
                ...
                <nav className="sideNav" aria-label="页面切换">
                    <button
                        type="button"
                        className={
                            screen === "game" ? "navBtn active" : "navBtn"
                        }
                    ...
                    <button
                        type="button"
                        className={
                            screen === "compendium" ? "navBtn active" : "navBtn"
                        }
            ...
            <section className="workspace">
                ...
                {screen === "game" ? (
                    <>
                        ...
                        >
                            {REGION_TABS.map((tab) => {
                                const locked = !unlockedRegions.includes(
                                    tab.id,
                                );
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span className="regionTabLabel">
                                                {tab.label}
                                            </span>
                                            {locked ? (
                                                <span className="regionTabBadge regionTabBadgeLocked">
                                                    未解锁
                                                </span>
                                            ) : (
                                                <span className="regionTabBadge">
                                                    {plotCount} 格
                                                </span>
                                            )}
                                        </button>
                        ...
                        {warehouseOpen && (
                            ...
                            >
                                <div
                                    className="modalBox warehouseModal"
                                    role="dialog"
                                    aria-label="仓库"
                                    onClick={(e) => e.stopPropagation()}
                                >
